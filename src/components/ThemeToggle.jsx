...
export const ThemeToggle = () => {
  ...
  return (
    <button
      type="button"
      onClick={toggleTheme}
      className={`w-full flex items-center justify-between px-3 py-2 text-xs font-medium rounded border transition-all duration-150 ${
        isDark
    ...
    >
      ...
      <span
        className={`inline-block w-8 h-4 rounded-full p-0.5 transition-colors ${
          isDark ? 'bg-amber-400' : 'bg-gray-300'
        }`}
      >
        <span
          className={`block w-3 h-3 rounded-full bg-white shadow-xs transform transition-transform duration-150 ${
